...
public protocol VideoPickerDelegate: class {
    func didSelect(url: URL?, info: [UIImagePickerController.InfoKey : Any]?)
}
...
open class VideoPicker: NSObject {
    ...
    private func pickerController(_ controller: UIImagePickerController, didSelect url: URL?, info: [UIImagePickerController.InfoKey : Any]?) {
        controller.dismiss(animated: true, completion: nil)

        self.delegate?.didSelect(url: url, info: info)
    }
}
...
//        }
        self.pickerController(picker, didSelect: url, info: info)
    }
}
